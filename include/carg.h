...
CARG_EXPORT bool internal_carg_has_flag   (uint64_t  item, uint64_t flag);
CARG_EXPORT void internal_carg_set_flag   (uint64_t *item, uint64_t flag);
CARG_EXPORT void internal_carg_clear_flag (uint64_t *item, uint64_t flag);
CARG_EXPORT void internal_carg_toggle_flag(uint64_t *item, uint64_t flag);
    
CARG_EXPORT int   internal_carg_test_printf           (const char *formatter, ...);
CARG_EXPORT int   internal_carg_test_vsnprintf        (const char *formatter, va_list args);
CARG_EXPORT int   internal_carg_secure_sprintf_concat (char *startPointer, char *endPointer, char **cursor, const char *formatter, ...);
CARG_EXPORT int   internal_carg_secure_vsprintf_concat(char *startPointer, char *endPointer, char **cursor, const char *formatter, va_list argsToCopy);
...
CARG_EXPORT void internal_carg_print_positional_usage_buffer_ts    (CargContext *cargLocalContext);
CARG_EXPORT void internal_carg_print_non_positional_usage_buffer_ts(CargContext *cargLocalContext);

CARG_EXPORT bool internal_carg_adjust_multi_arg_setter_ts(const CargContext *cargLocalContext, CargArgContainer *currentArg, void **varDataPtr);
CARG_EXPORT bool internal_carg_adjust_named_assign_ts    (const CargContext *cargLocalContext, int argIndex, const char *formatToken, const char *flagToken, const char **argToFormat, char *argumentFlagToCompare);
...
CARG_EXPORT void internal_carg_reference_grouped_boolean_arg_formatter_ts(const CargContext *cargLocalContext, int i, size_t j, const char *noPrefixFormat, bool **varDataPtr, va_list args);

CARG_EXPORT void internal_carg_set_named_arg_ts   (const CargContext *cargLocalContext, CargArgContainer *currentArg, void **varDataPtr, int argIndex, const char *formatToken, const char *argToFormat, char **formatToTokenize, char **argumentFlagToCompare);
CARG_EXPORT int  internal_carg_set_nested_arg_ts  (const CargContext *cargLocalContext, CargArgContainer *currentArg);
CARG_EXPORT void internal_carg_set_env_defaults_ts(const CargContext *cargLocalContext, char **stringToTokenize, char **tokenSavePointer, void **stringAllocation, va_list args);
